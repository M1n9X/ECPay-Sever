...
func openDailyLogLocked() error {
	...
	logFileName := fmt.Sprintf("tcb-server.%s.log", today)
...
func cleanupOldLogs() {
	...
	for _, file := range files {
		...
		name := file.Name()
		// Match pattern: tcb-server.YYYY-MM-DD.log
		if !strings.HasPrefix(name, "tcb-server.") || !strings.HasSuffix(name, ".log") {
			continue
		}

		// Extract date from filename
		datePart := strings.TrimPrefix(name, "tcb-server.")
