...
设备型号差异会影响回传字段完整性。PDF 未列出机型清单，因此必须以**供应商确认 + 验收测试**为准。  
...
**ECRTool 验证结论**：  
官方 `NEXSYS ECRTool Ver2.3-Log` 样例回传中，以上字段均有回传，说明**正常设备/正常配置应回传关键字段**。  
若现场设备不回传，多为设备配置或机型限制，建议视为不合格机型。

**验收测试清单**：
1. 一般销售：确认 `Invoice_No / Reference_No / Approval_No / Card_No` 均回传  
2. 一般退货：确认回传字段与销售一致  
3. 隔日退货：验证主机是否要求参照字段  
4. 分期退货 / 金融卡退货：验证 required 字段是否回传  
